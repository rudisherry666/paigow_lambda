...
function validateSession(dynamodb, sessionHash) {
    ...
    if (!sessionHash) {
        ...
        response = { error: 'You must be logged in to perform this action', code: 'PG_ERROR_NO_SESSION' };
        defer.reject(response);
    } else {
        ...
        .then(function(session) {
            if (!session || session.sessionHash !== sessionHash) {
                ...
                response = { error: 'Cannot get session', code: 'PG_ERROR_DB_SESSION_NOT_FOUND' };
                defer.reject(response);
            } else {
...
function putItem(dynamodb, tableName, item) {
    ...
    dynamodb.putItem(params, function(err, data) {
        if (err) {
            ...
            response = { error: 'Cannot write to table' + tableName, code: 'PG_ERROR_DB_PUTITEM' };
            defer.reject(response);
        }
        defer.resolve();
    });
